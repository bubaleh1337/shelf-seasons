# Shelf Seasons 0.12.0 — дневник чтения на Windows
...
2. Если миграции прошлых версий ещё не запускались, примените их по порядку из
   `supabase/migrations`.
3. Для обновления с версии 0.11.0 на 0.12.0 новая SQL-миграция не нужна.
...
После входа откройте Настройки и проверьте кнопку Buy Me a Coffee, email и
Telegram. На телефоне Настройки открываются кнопкой-шестерёнкой сверху.

## 4. Отправьте обновление в GitHub

```powershell
git add -A
git commit -m "feat: add developer contacts and project support"
