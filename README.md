...
Version 0.19.0 includes the private personal library, reliable Google OAuth and in-app locale switching, resilient Russian and English book search with browser-side catalog fallbacks, provider and custom covers, current-page reading progress, readable calendar cards, streaks, explicit rereading, book completion, compact inspectable yearly goals, user-owned ordered series, physical seasonal shelves with spine-first books, responsive original seasonal backgrounds, protected book-text surfaces, deduplicated monthly/yearly recaps with whole-library category choices, reading-language history, developer contacts, project support, account export and protected account deletion. Home library-preview cards open the same book editor as the full Library. The public beta also includes patched production dependencies, database-backed rate limits, browser security headers, bilingual legal pages, localized metadata and installable PWA icons. Initial data queries and private-cover links are batched for a smoother public beta, and the repository contains an encrypted daily database-backup workflow.
